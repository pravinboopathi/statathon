...
import APIService, { Survey, QueryFilter } from '../../services/apiService';
...
export const QueryBuilder: React.FC = () => {
  ...
  React.useEffect(() => {
    const loadSurveys = async () => {
      try {
        ...
        setSurveys(availableSurveys as any);
      }
    };
    loadSurveys();
  }, []);
  ...
  const handleNLToSQL = async () => {
    ...
    try {
      // Instead of calling translateNLToSQL which used the old API, 
      // we'll now directly query the natural language endpoint
      const response = await APIService.executeNaturalLanguageQuery(nlQuery, {});
      ...
      if (response.success) {
        ...
        if (response.data.insights) {
          setAiInsights(response.data.insights.summary || 'Analysis of query results');
        }
      } else {
        setQueryError(response.message);
      }
    } catch (error) {
      console.error('Natural language query failed:', error);
      setQueryError('Failed to process natural language query. Please try again.');
    } finally {
  ...
  const handleExecuteQuery = async () => {
    ...
    try {
      // Update to use the new direct query endpoint instead of SQL execution
      const response = await APIService.executeQuery(sqlQuery, {});
      ...
      setShowResults(true);
      setAiInsights("Analysis shows that the 25-29 age group has higher employment rates compared to the 20-24 group.");
    } finally {
        ...
        <Card>
          <CardContent>
            ...
            <FormControl fullWidth sx={{ mb: 2 }}>
              ...
              >
                {(surveys.length > 0 ? surveys : availableSurveys).map((survey) => (
